...
def add_pdf_task lang, description
  ...
  task :pdf => "clean:pdf" do
    cd lang do
      sh "review-pdfmaker ajhc-hg.yaml"
    end
  end

  namespace :clean do
    task :pdf do
      sh "rm -f ./#{ lang }/ajhc-hg.pdf"
    end
  end
...
def add_epub_task lang, description
  ...
  task :epub => "clean:epub" do
    cd lang do
      sh "review-epubmaker ajhc-hg.yaml"
    end
  end

  namespace :clean do
    task :epub do
      sh "rm -f ./#{ lang }/ajhc-hg.epub"
    end
  end
...
def add_mobi_task lang, description
  ...
  task :mobi => [ :epub, "clean:mobi" ] do
    ...
    cd lang do
      sh "KindleGen ajhc-hg.epub" rescue nil
    end
  end

  namespace :clean do
    task :mobi do
      sh "rm -f ./#{ lang }/ajhc-hg.mobi"
    end
  end
